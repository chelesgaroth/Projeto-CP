...
\textbf{Grupo} nr. & 96
\\\hline
a89472 & Fernando Henrique Carvalho Lopes 
\\
a93281 & Leonardo Araújo Freitas	
\\
a89574 & Pedro Almeida Fernandes	
\\
a89464 & Raquel Sofia Miranda da Costa 
\end{tabular}
\end{center}
...
\end{code}
Nesta alínea é suposto calcular o outExpAr e o recExpAr que correspondem respetivamente ao out da função e ao functor da mesma. Pela definição do tipo ExpAr A 
vemos que existem 4 construtores: X, N, Bin e Un. Através da definição da função inExpAr vemos o aninhamento que existe ao nível do coproduto, isto é 
|X + (N + (Bin + Un)))|. As expressões obtidas do outExpAr refletem esse mesmo aninhamento no que toca às injeções i1 e i2. Em relação à função recExpAr o grupo
teve o cuidado de procurar nas bibliotecas disponibilizadas uma função que exemplificasse o comportamento da recExpAr. 
Deste modo em seguida são definidas as mesmas:
...
---
\end{code}
De modo a conseguir obter o gene do catamorfismo recorreu-se ao seguinte diagrama:
...
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A) + UnOp >< ExpAr A))|
           \ar[d]^{|id + (id+(id >< (eval_exp x >< eval_exp x) + id >< eval_exp x))|}
\\
     |A quadrado|
&
     |1 + (A + (BinOp >< (A quadrado >< A quadrado) + UnOp >< A quadrado))|
           \ar[l]^-{|g_eval_exp x|}
}
\end{eqnarray*}
...
---
\end{code}
O clean e o gopt são dois genes que necessitamos deduzir. O gopt é o gene de um catamorfismo, sendo que pode ser igual ao |g_eval_exp|, já o clean, que é o gene do
anamorfismo foi necessário deduzir tendo em consideração que necessita "limpar" os casos absorventes.
\begin{code}
clean :: (Eq a, Num a) => ExpAr a -> Either () (Either a (Either (BinOp, (ExpAr a, ExpAr a)) (UnOp, ExpAr a)))
clean = outExpAr . either g1 (either g2 (either g3 g4)) . outExpAr where
  g1 = const X
  g2 = N
  g3 (Product, (N 0, b)) = N 0
  g3 (Product, (a,N 0)) = N 0
  g3 (Sum, (a,N 0)) = a
  g3 (Sum, (N 0, b)) = b
  g3 (op, (a, b)) = Bin op a b
  g4 (op, a) = Un op a
...
\end{code}
Tendo em conta que a função sd recebe um ExpAr e devolve um outro, e considerando o tipo já definido do gene do catamorfismo |sd_gen|, foi
necessário recorrer aos construtores do tipo ExpAr para conseguir definir tanto a regra da soma como a do produto das derivadas. Assim sendo, foi possível
dada uma expressão aritmética, calcular a respetiva derivada.
...
sd_gen = split fun deriv where
    fun = either g1 (either g2 (either g3 g4)) where
        g1 = const X 
        g2 = N 
        g3 (binop, ((a,b),(c,d))) = Bin binop a c
        g4 (unop, (a,b)) = Un unop a
    deriv = either g1 (either g2 (either g3 g4)) where
        g1 = const (N 1)
        g2 = const (N 0)
        g3 (Sum, ((a,b),(c,d))) = Bin Sum b d
        g3 (Product, ((a,b),(c,d))) = Bin Sum (Bin Product a d) (Bin Product b c)
        g4 (E, (a,b)) = Bin Product (Un E a) b 
        g4 (Negate, (a,b)) = Un Negate b  
        
\end{code}
Nesta alínea de modo a simplificar o problema realizamo-lo apenas com um either de splits (aplicando a lei da troca) em vez de repetirmos o processo
da alínea anterior, ou seja, um split de eithers. É importante referir que neste caso o raciocínio foi ligeiramente modificado do aplicado na 
alínea anterior, devido ao tipo de |ad|, que agora deverá devolver apenas |a| (neste caso um número), enquanto que no anterior todas as operações 
devolviam ExpAr's. Assim sendo, dada uma expressão aritmética e um ponto, é calculado o valor da derivada nesse mesmo ponto sem recorrer ao método que envolve
a manipulação da expressão original.
\begin{code}
...
cat = prj . for loop inic where
 loop (cat,c,b) = (cat * c `div` b, c + 4 , b + 1)
 inic = (1,2,2)
 prj (cat,c,b) = cat
...
~\newline\newline
De modo a descobir cat (n + 1) sucedeu-se ao seguinte calculo matemático:
~\newline\newline
\begin{array}{lll}
cat (0 = 1\\
\\
cat (n + 1) = \frac{(2(n+1))!}{(n+2)!\times(n+1)!}\\
\\
cat (n + 1) = \frac{(2n+2)!}{(n+2)!\times(n+1)!}\\
\\
cat (n + 1) = \frac{(2n+2)\times(2n+1)\times(2n)!}{(n+2)!\times(n+1)!}\\
\\
cat (n + 1) = \frac{(2n+2)\times(2n+1)\times(2n)!}{(n+2)\times (n+1)!\times(n+1)\times(n)!}\\
\\
cat (n + 1) = \frac{2\times(n+1)\times(2n+1)\times(2n)!}{(n+2)\times (n+1)!\times(n+1)\times(n)!}\\
\\
cat (n + 1) = \frac{2\times(2n+1)}{n+2} \times cat (n)\\
\\
cat (n + 1) = \frac{4n+2}{n+2} \times cat (n)\\
\end{array}
~\newline\newline

Sendo c o numerador da fração e b o denominador dessa mesma fração, obtemos o seguinte:
~\newline\newline
Dedução da função c:
...
calcLine = cataList h where
  h = either g1 g2  where
  g1 () = const nil
  g2 (a,b) [] = nil
  g2 (a,b) (c:cs) = z1 where
      z1 z = concat $ sequenceA [singl.linear1d a c, b cs] z  
  
   
deCasteljau :: [NPoint] -> OverTime NPoint
deCasteljau = hyloAlgForm alg coalg where
  coalg = undefined --either g1 g2  where               ----- coalg é anaformismo ,    alg é catamorfismo 
   -- g1 () = nil
   -- g1 [p] =  i1 . p
   -- g2 l = i1 . i2 l 
  alg = undefined --either j1 j2 where
   -- j1 () = nil
   -- j2 [p] = const p
   -- j2 (p,q) l = \pt -> (calcLine (p pt) (q pt)) pt


hyloAlgForm = undefined -- alg . coalg 
\end{code}

\subsection*{Problema 4}

Prova avg\_aux:
\item |avg_aux = cata (either b q)| \par
\item \equiv |{ avg_aux = split avg length }|\par
\item |split avg lenth = cata (either b q)|\par
\item
\item \equiv |{ Inferir tipo de b e q  }|\par
\item  |split avg length = cata (either (split b1 b2) (split q1 q2))|\par
\item
\item \equiv |{ Lei da troca}|\par
\item  |split avg length = cata (split (either b1 q1) (either b1 q2))|\par
\item
\item \equiv |{ Fokkinga}|\par
\item  |avg . in = either b1 q1 . F (split (avg) (length))|\par
\item  |length . in = either b2 q2 . F (split (avg) (length))|\par
\item
\item \equiv |{ Listas: F f = id + id  f}|\par
\item |avg . in = either b1 q1 . (id + id >< split avg length)|\par
\item |length . in = either b2 q2 . (id + id >< split avg length)|\par
\item
\item \equiv |{  in = either singl cons }|\par
\item |avg . either singl cons = either b1 q1 . (id + id >< split avg length)|\par
\item |length . either singl cons = either b2 q2 . (id + id >< split avg length)|\par
\item
\item \equiv |{ fusão-+ lado esquerdo; absorção-+ }| \par
\item |either (avg.singl)  (avg.cons) = either (b1 . id) (q1 . id >< (split avg length))| \par
\item |either (length.singl) (length.cons) = either (b2 . id) (q2 . id >< (split avg length))| \par
\item
\item \equiv |{ Natural - id }| \par
\item |either (avg.singl) (avg.cons) = either (b1) (q1 . id >< split avg length)| \par
\item |either (length.singl) (length.cons) = either(b2) (q2 . id >< split avg length)| \par
\item
\item \equiv |{Eq -+}| \par
\item |avg.singl = b1| \par
\item |avg.cons = q1 . id >< split avg length| \par
\item |length.singl = b2| \par
\item |length.cons = q2 . id >< split avg length| \par
\item
\item \equiv |{Def-comp 2x}| \par
\item |avg (singl a) = b1 a| \par
\item |avg (cons (a,b)) = q1(id >< split avg length) (a,b)| \par
\item |length (singl a) = b2 a| \par
\item |length (cons (a,b)) = q2(id >< split avg length) (a,b)| \par
\item
\item \equiv |{Def-x}| \par
\item |avg (singl a) = b1 a| \par
\item |avg (cons (a,b)) = q1(id a >< split avg length b)| \par
\item |length (singl a) = b2 a| \par
\item |length (cons (a,b)) = q2(id a >< split avg length b)| \par
\item
\item \equiv |{ }| \par
\item |avg (singl a) = b1 a| \par
\item |avg (cons (a,b)) = q1(a, (avg b, length b))| \par
\item |length (singl a) = b2 a| \par
\item |length (cons (a,b)) = q2(a, (avg b, length b))| \par
\item
\item \equiv |{ }| \par 
\item |avg [a] = b1 a| \par
\item |avg (a:b) = q1(a, (avg b, length b))| \par
\item |length [a] = b2 a| \par
\item |length (a:b) = q2(a, (avg b, length b))| \par
...
avg_aux = cataList' gene_aux where
  gene_aux = (either init loop) 
  loop = split q1 q2
  init = split b1 b2
  b1 n = id n
  b2 n = 1
  q1 (a, (avg_b, length_b)) =  (a + avg_b * length_b) / (length_b + 1)
  q2 (a, (avg_b, length_b)) = length_b + 1


\end{code}

Prova para árvore do tipo LTree: 
...
\item
\item \equiv |{ Listas: F f = id + id  f}|\par
\item |avg . in = either b1 q1 . (id + (split avg length)^2)|\par
\item |length . in = either b2 q2 . (id + (split avg length)^2)|\par
\item
\item \equiv |{  in = either Leaf Fork }|\par
\item |avg . either Leaf Fork = either b1 q1 . (id + split avg length >< split avg length)|\par
\item |length . either Leaf Fork = either b2 q2 . (id + split avg length >< split avg length)|\par
\item
\item \equiv |{ fusão-+ lado esquerdo; absorção-+ }|\par
\item |either (avg.Leaf) (avg.Fork) = either (b1.id) (q1 . split avg length >< split avg length)|\par
\item |either (length.Leaf) (length.Fork)length . either Leaf Fork = either (b2.id) (q2 . split avg length >< split avg length)|\par
\item
\item \equiv |{ Natural - id }|\par
\item |either (avg.Leaf) (avg.Fork) = either (b1) (q1 . split avg length >< split avg length)|\par
\item |either (length.Leaf) (length.Fork)length . either Leaf Fork = either (b2) (q2 . split avg length >< split avg length)|\par
\item
\item \equiv |{ Eq -+ }|\par
\item |avg.Leaf = b1|\par
\item |avg.Fork = q1 . split avg length >< split avg length |\par
\item |length.Leaf = b2|\par
\item |length.Fork = q2 . split avg length >< split avg length |\par
\item
\item \equiv |{ Def-comp 2x }|\par
\item |avg(Leaf a) = b1 a|\par
\item |avg(Fork(LTree a, LTree b)) = q1 (split avg length >< split avg length) (LTree a, LTree b) |\par
\item |length(Leaf a) = b2 a|\par
\item |length(Fork(LTree a, LTree b)) = q2 (split avg length >< split avg length) (LTree a, LTree b) |\par
\item
\item \equiv |{ Def-x }|\par
\item |avg(Leaf a) = b1 a|\par
\item |avg(Fork(LTree a, LTree b)) = q1 (split avg length (LTree a), split avg length(LTree b))|\par
\item |length(Leaf a) = b2 a|\par
\item |length(Fork(LTree a, LTree b)) = q2 (split avg length (LTree a), split avg length(LTree b))|\par
\item
\item \equiv |{ }|\par
\item |avg(Leaf a) = b1 a|\par
\item |avg(Fork(LTree a, LTree b)) = q1 ((avg(LTree a),length(LTree a)), (avg(LTree b),length(LTree b)))|\par
\item |length(Leaf a) = b2 a|\par
\item |length(Fork(LTree a, LTree b)) = q2 ((avg(LTree a),length(LTree a)), (avg(LTree b),length(LTree b)))|\par
...
avgLTree = p1.cataLTree' gene where
    ...
    b1 a = id a
    b2 a = 1
    q1 ((aa,al),(ba,bl)) =  (( aa * al) + ( ba * bl)) / (al + bl)
    q2 ((aa,al),(ba,bl)) = al + bl
\end{code}
